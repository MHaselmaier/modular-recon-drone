...
Aufgrund der schlichten Schnittstelle zur Drone ist es sehr einfach, eine
beliebige Steuerung zu implementieren. Um die Drone anzusteuern, muss sich 
zuerst mit dem bereitgestellt W-Lan-Netzwerk verbunden werden. Innerhalb
dieses Netzwerkes hat die Drone immer die IP-Addresse _192.168.4.1_. Um 
das Kamerasignal zu erhalten, muss eine TCP-Verbindung zur Drone über den 
Port _1234_ aufgebaut werden. Die Drone sendet dann das Bildsignal Byteweise
...
Wert im Intervall [0, 100]. Um die Drone um Hindernisse zu manövrieren, muss die 
Geschwindigkeit der beiden Motoren unterschiedlich hoch definiert werden.

Im Folgenden wird eine Referenzimplementierung der Steuerung der Drone am Beispiel
